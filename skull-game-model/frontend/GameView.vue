  ...
  >
    ...
    <main class="scene-stage" aria-label="骷髅牌沉浸式牌桌">
      ...
      >
        ...
        <footer class="seat-counters">
          <span>{{ player.handCount }} 手持</span>
          <span>{{ player.stack.length }} 已叠</span>
          <span>{{ player.removedCount }} 失去</span>
        </footer>
...
.table-disc.revealed { transform: translate(-50%, calc(-50% - var(--disc-rise))) rotateY(0); }
...
.history-drawer { position: absolute; z-index: 18; right: clamp(12px,1.8vw,26px); top: clamp(12px,1.8vw,24px); width: min(290px,31%); border: 1px solid rgba(255,255,255,.08); border-radius: 14px; background: rgba(11,14,12,.82); backdrop-filter: blur(16px); overflow: hidden; }.drawer-toggle { width: 100%; display: grid; grid-template-columns: auto 1fr auto; align-items: center; gap: 8px; border: 0; padding: 11px 12px; color: var(--skull-muted); background: transparent; cursor: pointer; text-align: left; }.drawer-toggle b { color: var(--skull-ink); font-size: 9px; }.history-drawer ol { max-height: 245px; display: grid; gap: 0; margin: 0; padding: 0 12px 10px; overflow: auto; list-style: none; }.history-drawer li { display: grid; grid-template-columns: 8px 1fr; gap: 7px; padding: 7px 0; border-top: 1px solid rgba(255,255,255,.055); }.history-drawer p { margin: 0; color: var(--skull-muted); font-size: 7px; line-height: 1.45; }.event-dot { width: 6px; aspect-ratio: 1; margin-top: 3px; border-radius: 50%; background: #797d78; }.event-dot.reveal,.event-dot.challenge_success { background: var(--skull-safe); }.event-dot.penalty_pending,.event-dot.eliminated,.event-dot.last_chance_elimination { background: var(--skull-danger); }
...
.rules-content { display:grid; gap:2px; }.rules-content section { display:grid; grid-template-columns:35px 1fr; gap:12px; padding:13px 0; border-bottom:1px solid var(--line); }.rules-content section > span { width:33px; aspect-ratio:1; display:grid; place-items:center; border:1px solid color-mix(in srgb,var(--gold) 35%,var(--line)); border-radius:50%; color:var(--gold); font-family:Georgia,serif; font-size:12px; }.rules-content h3 { margin:0 0 4px; font-size:12px; }.rules-content p { margin:0; color:var(--muted); font-size:9px; line-height:1.65; }.rules-content footer { margin-top:10px; border-radius:10px; padding:11px; color:var(--muted); background:var(--surface-inset); font-size:8px; line-height:1.6; }
...
@media (prefers-reduced-motion: reduce) { .bid-orbit,.stack-zone.clickable { animation:none; }.table-disc,.player-seat,.hand-disc { transition:none; } }
...
@media (max-width: 980px) {
  ...
  .ritual-table,.table-shadow { width:78%; height:66%; }.player-seat { width:clamp(130px,18vw,172px); }.seat-nameplate { grid-template-columns:30px minmax(0,1fr); }.player-avatar { width:30px; }.win-track { position:absolute; right:6px; bottom:-11px; }.phase-banner,.history-drawer { width:min(260px,38%); }
