...
const openingPieceSize = computed(() => me.value?.openingPieceSize ?? null)
const previewError = computed(() => !selected.value ? '先选一块棋块，再点击棋盘定位'
  : openingPieceSize.value !== null && selected.value.size !== openingPieceSize.value
    ? `初始顺位前两位的首块必须选择 ${openingPieceSize.value} 格棋块`
...
function pieceAllowed(piece: Piece): boolean {
  return openingPieceSize.value === null || piece.size === openingPieceSize.value
}
...
function selectPiece(piece: Piece): void {
  if (!canPreview.value || !me.value?.remainingPieces.includes(piece.id) || !pieceAllowed(piece) || pending.value) return
  ...
  <section class="blokus" :class="[me?.color ?? 'blue', { duo }]" aria-label="方格游戏对局">
    <div class="status-board">
      ...
      <div class="turn-strip" :class="{ mine: canAct }" role="status">
        ...
        <div>
          ...
          <span>{{ finished ? '按剩余方格数从少到多排名' : firstMove && canAct ? openingPieceSize ? `首块必须用 ${openingPieceSize} 格棋块覆盖起始点` : '第一块必须覆盖你的起始点' : '同色角接，不可边接；不同颜色可以相邻' }}</span>
        </div>
        <small>已落 {{ game.moveCount ?? 0 }} 块 · 无落子时限</small>
      </div>
    </div>
    ...
    <div class="workbench">
      ...
      <div class="side-column">
        ...
        <aside class="inventory" :class="inspected?.color ?? 'blue'" aria-label="棋块库">
          ...
          <p class="inventory-note">{{ !ownTray ? '公开棋块库 · 仅供查看' : openingPieceSize ? `你是初始顺位前两位，首块只能选择 ${openingPieceSize} 格棋块。` : canPreview ? '选择一块，旋转或翻转后放到棋盘上。' : '查看本局剩余棋块。' }}<br />每人共 89 小格，灰色棋块已使用。</p>
          ...
          <div v-for="size in sizeGroups" :key="size" class="piece-group">
            ...
            <div class="piece-grid">
              <button v-for="piece in PIECES.filter(piece => piece.size === size)" :key="piece.id" type="button" class="piece-button"
                :class="{ selected: selectedId === piece.id && ownTray, used: !inspected?.remainingPieces.includes(piece.id), 'opening-locked': ownTray && !pieceAllowed(piece) }"
                :disabled="!canPreview || pending || !inspected?.remainingPieces.includes(piece.id) || !pieceAllowed(piece)"
                :aria-label="`${piece.id}，${size} 格${!inspected?.remainingPieces.includes(piece.id) ? '，已使用' : ownTray && !pieceAllowed(piece) ? '，首步不可用' : ''}`"
    ...
    <PluginModal v-if="showRules" title="方格游戏 · 玩法" aria-label="双人或四人方格玩法规则" size="medium" mobile-sheet @close="showRules = false">
      <div class="rule-content">
        ...
        <section><h3>每回合放一块</h3><ol><li>首块必须覆盖分配给自己的起始点。四人局的初始顺位第 1、2 位首块必须使用恰好 4 格的棋块；第 3、4 位可以使用 5 格棋块，也可以选择其他大小。双人局不限制首块大小。</li><li>之后每块至少与一块自己的棋块角接；同色绝不能边接。不同颜色可以边接或角接。</li><li>可以旋转、翻转，但不能重叠、越界或移动已落下的棋块。</li><li>选择棋块后点击棋盘定位，用旋转、翻转和方向按钮微调，再点「确认落子」。小圆点提示可角接的空格，「找落点」可寻找当前棋块的一个合法位置。</li><li>只有全部剩余棋块都无处可放，才会自动跳过；不能主动跳过。已放完的玩家也自动跳过，其他人继续。</li><li>本游戏没有落子倒计时，等待多久都不会因超时弃权；断线也不会自动弃权，在房间存续期间保留座位和当前回合，重连后可继续。</li></ol></section>
        <section><h3>终局与名次积分</h3><p>所有人都放完或无处可放时结束。按剩余小方格数从少到多排名。2 人局依次为 +2、+1；4 人局依次为 +2、+1、0、−1。</p><p>四人局只要剩余格数相同，本局随机初始顺位较后的玩家就直接排在前面，作为后手补偿。双人局同分时先比较剩余块数，块数仍相同再由初始顺位较后的玩家优先。不使用官方版本的放完奖励分。</p><p>只有玩家主动认输并退出才视为弃权，已落棋块保留。弃权者排在其他玩家之后，多人弃权时越早弃权名次越后；仅剩一位未弃权玩家时立即结算。</p><p>单局名次分保存在战绩说明中。大厅排行榜沿用胜负规则，不累计名次分。含游客的对局可以游玩，但不记录账号战绩，也不计入大厅排行榜。</p></section>
