...
const nextMap = computed(() => game.value.mapCatalog.find(map => map.key === game.value.nextMap) ?? null)
  ...
  <section ref="gameRoot" class="bomb-people" :class="[`stage-${game.stage}`, { fullscreen: isFullscreen }]">
    ...
    <section v-if="isNegotiationPhase && showMaps" class="map-negotiation" aria-label="地图协商">
      <div class="negotiation-heading">
        <div><p class="eyebrow">MAP CONSENSUS</p><h3>房主提议，全员确认后锁定下一局地图</h3></div>
        ...
        <p v-else-if="nextMap" class="proposal-status">下一局已锁定：<strong>{{ nextMap.name }}</strong><span>全员已经同意</span></p>
        <p v-else class="proposal-status">{{ snapshot.phase === 'finished' ? '本局地图' : '当前展示' }}：<strong>{{ selectedMap?.name }}</strong><span>未协商则开局时随机抽取</span></p>
      </div>
      ...
      <div v-if="game.mapProposal && proposedMap" class="approval-strip">
        ...
        <div v-if="game.canVoteMap" class="vote-actions">
          ...
          <PluginButton :disabled="votePending" @click="voteMap(true)"><Check :size="16" />同意该地图</PluginButton>
        </div>
      ...
      <div class="map-grid">
        <button
          ...
          type="button"
          class="map-card"
          :class="{ selected: map.key === game.selectedMap, proposed: map.key === game.mapProposal?.mapKey, queued: map.key === game.nextMap }"
          :disabled="!game.canProposeMap || Boolean(mapPending)"
          :aria-label="`${map.name}，${map.pace}，${map.density}${map.key === game.mapProposal?.mapKey ? '，待确认地图' : map.key === game.nextMap ? '，下一局已锁定地图' : map.key === game.selectedMap ? '，本局地图' : ''}`"
        ...
        >
          ...
          <span v-if="map.key === game.mapProposal?.mapKey" class="selected-mark proposed-mark"><Timer :size="13" />待确认</span>
          <span v-else-if="map.key === game.nextMap" class="selected-mark queued-mark"><Check :size="13" />下局</span>
          <span v-else-if="map.key === game.selectedMap" class="selected-mark"><Check :size="13" />{{ snapshot.phase === 'finished' ? '本局' : '展示' }}</span>
        </button>
      </div>
      <p class="map-help">
        {{ game.canProposeMap ? '点击地图发起或更换提议；所有当前玩家同意后，下一局就使用该地图。' : game.mapProposal ? '只有房主能发起提议；当前玩家可以同意或否决。' : '等待房主发起提议；没有全员通过的提议时，服务端仍会随机抽图并排除上一局。' }}
      </p>
    </section>
    ...
    <section v-if="snapshot.phase === 'lobby'" class="lobby-overview">
      <div class="lobby-copy">
        ...
        <h3>{{ nextMap?.name ?? '下一局默认随机地图' }}</h3>
        <p>{{ nextMap ? `${nextMap.subtitle}。该地图已经全员同意，下一局将直接使用。` : `房主可在上方发起地图协商；若没有全员通过的提议，开局时会从全部 ${game.mapCatalog.length} 张地图中随机抽取并避免与上一局重复。` }}每局先对抗 90 秒，随后从左上角开始沿边缘顺时针一圈圈落石，最后生还者夺冠。</p>
        <div class="rule-chips"><span>2–8 人</span><span>全员协商选图</span><span>炸弹 2 秒</span><span>自动拾取</span><span>落石可淘汰</span></div>
        <small>地图协商完成或采用随机地图后，由房主使用房间顶部的“开始游戏”。</small>
      </div>
    ...
    <div v-else class="play-layout">
      ...
      <aside class="battle-hud">
        <section v-if="snapshot.phase === 'finished'" class="result-card panel">
          ...
          <PluginButton variant="secondary" block @click="showMaps = !showMaps"><MapIcon :size="16" />协商下一局地图</PluginButton>
        </section>
        ...
        <section class="controls panel">
          ...
          <p>普通炸弹达到容量后再按空格，会用额外槽部署一枚遥控定时炸弹；它不会倒计时或被火焰、连锁引爆，只有所有者按 C 才会爆炸。角色位于两格之间时，放雷和面前交互始终按朝向取身体前方那一格。扔雷分两步：第一次拿起面前炸弹，可抱着继续移动；第二次按当前朝向投出最多四格。抱雷期间不能放雷或遥控引爆，普通炸弹的剩余引信暂停，投出或落地后继续倒计时。幽灵相位获得后本局永久生效，可穿过箱墙、炸弹和其他玩家并在箱墙格内放雷，但不能穿固定石块或决胜落石。</p>
        </section>
    ...
    <PluginModal v-if="showRules" title="炸弹超人 · 完整玩法" size="large" mobile-sheet @close="showRules = false">
      <div class="rulebook">
        ...
        <section><h3>炸弹与通用交互</h3><p>普通炸弹放下后最多 2 秒爆炸，火焰按十字方向传播，固定石块和落石会阻挡，可破坏箱墙被摧毁后有 38% 概率掉落道具。角色位于两格之间时，炸弹会放在朝向前方的那一格，离开重叠范围前也不会被自己的炸弹卡死。幽灵相位获得后本局永久生效，可穿箱墙、炸弹和其他玩家并在墙内放雷；墙内炸弹爆炸时会同时摧毁所在箱墙，但幽灵不能穿固定石块或决胜落石。火焰和连锁引线只会引爆普通炸弹；遥控定时炸弹只响应所有者的 C 键。</p></section>
        <section><h3>电脑键盘与手机触屏</h3><ul><li><kbd>W A S D</kbd> 或方向键：连续移动，松键可以停在两格之间，快速换向会平滑转弯。</li><li><kbd>Space</kbd>：先放普通炸弹；身体在中线时固定放到朝向前方格。普通容量已满且拥有遥控定时炸弹时，再部署一枚不占普通容量的遥控雷。</li><li><kbd>C</kbd>：引爆自己的遥控定时炸弹；引爆后额外槽永久保留，可再次部署。</li><li><kbd>Z</kbd>：用拳击手套即时把身体前方炸弹打出三格。</li><li><kbd>X</kbd>：第一次拿起身体前方炸弹，之后可继续移动；第二次按当前朝向把手中炸弹越过障碍投到前方最多四格。若没有可用落点则继续抱着。</li><li>抱雷会占用双手，不能放雷、遥控引爆或使用拳套；普通炸弹被拿起时冻结剩余引信，成功投出，或因退出、阵亡、失去手套而落地后，才继续倒计时。</li><li>脚踢雷无需单独按键，朝炸弹移动即可让它以普通滚动两倍的速度持续向前。</li><li>手机端拖动左侧摇杆移动，右手点击拳击、放雷、遥控引爆和拿雷/投雷按钮。</li></ul></section>
        ...
        <section><h3>协商地图与战绩</h3><p>开局前或每局结束后，房主可以提议下一张地图；所有当前玩家都同意后即锁定给下一局。没有达成一致时，服务端会从全部地图中随机抽取，并排除上一局以避免连续重复。每名玩家记录本局击杀、房间累计击杀、夺冠数和胜率；结算后还会把胜负及击杀明细写入大厅战绩。</p></section>
      </div>
    </PluginModal>
  </section>
</template>
...
.rulebook { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; color: #171411; font-size: 12px; line-height: 1.8; }.rulebook section { padding: 14px; border: 1px solid #4b39252e; border-radius: 12px; background: #fffaf2; }.rulebook h3 { margin: 0 0 6px; color: #18120d; font-size: 15px; }.rulebook p, .rulebook ul { margin: 0; color: #171411; }.rulebook ul { padding-left: 20px; }.rules-items { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 5px; margin-bottom: 8px; }.rules-items article { min-width: 0; display: grid; justify-items: center; gap: 2px; }.rules-items img { width: 42px; height: 42px; object-fit: contain; }.rules-items strong { max-width: 100%; overflow: hidden; color: #171411; font-size: 8px; text-overflow: ellipsis; white-space: nowrap; }
