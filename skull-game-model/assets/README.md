...
这些 SVG 都是为软件建模新绘制的中性占位图，不包含官方 Logo、牌面图案、部落插画或扫描素材。它们可以用于评审信息架构和交互关系；正式插件仍需根据大厅的四套主题、768 x 768 目录图标规范和移动端布局重新制作运行资产。
...
大厅图标由 `scripts/generate_catalog_artwork.py` 重建。深浅版使用完全相同的牌、基座和镜头，只切换背景与材质；最终运行时仅保留两张 WebP。
