...
stateDiagram-v2
    ...
    bidding --> bidding: 加价或退出
    bidding --> reveal: 只剩最高叫价者
...
| `BidState` | 当前叫价、最高叫价者、已退出玩家 | 竞标推进与合法性判断 |
...
| `raise_bid` | `bidding` | `{ "count": 4 }` | 严格提高叫价并更新最高叫价者 |
| `pass_bid` | `bidding` | `{}` | 把玩家加入本轮永久退出集合；只剩一人时进入挑战 |
...
| `bid.raise-or-pass` | 中央叫价盘、当前最高者、已退出标记 | 加价或退出 |
...
- 每个座位包含：昵称、在线状态、成功次数、玩家垫、叠牌数量、是否退出、本轮最后机会标记。
...
12. 第二次挑战成功立即结束；只剩一名有效玩家也立即结束。
13. 最后机会牌只存在于获得后的下一轮，过期归还，每人最多获得一次。
14. 最后机会持有者在生效轮挑战失败时自动淘汰，不进入普通盲选。
15. 重连后的安全视图不包含他人手牌、暗置牌、秘密弃牌或处罚槽位映射。
