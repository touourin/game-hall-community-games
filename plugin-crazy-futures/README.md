...
在已经初始化 `community_games` 子模块的游戏大厅根目录执行：

```bash
.venv/bin/python -m pytest community_games/plugin-crazy-futures/tests
.venv/bin/python -m pytest backend/tests/test_game_plugins.py
npm --prefix frontend run test:run -- ../community_games/plugin-crazy-futures/frontend
